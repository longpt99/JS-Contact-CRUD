...
function main() {
  ...
  function newContact() {
    ...
    function saveContact() {
      let save = readlineSync.question(
        'Do you want to save this contact?(yes/no): '
      );
      switch (save) {
        case 'yes':
        case 'YES':
          ...
          phoneList.push(user);
          phoneList = JSON.stringify(arrangeId(phoneList));
  ...
  function editContact() {
    showContact();
    const id = readlineSync.question('Chose a id contact need to edit: ');
  ...
  function delContact() {
    showContact();
    const id = readlineSync.question('Chose a id contact to delete: ');
    const userIndex = phoneList.findIndex(function (value) {
      return value.id === +id;
    });

    if (userIndex !== -1) {
      phoneList.splice(userIndex, 1);
      phoneList = JSON.stringify(arrangeId(phoneList));
      fs.writeFileSync('./db.json', phoneList, { encoding: 'utf-8' });
  ...
  function searchContact() {
    let search = readlineSync.question('Type to search contact(Name/Number): ');
    if (!isNaN(search)) {
      ...
      let result = phoneList.filter(function (value) {
        return (
          change_alias(value.name)
            .toLowerCase()
            .indexOf(change_alias(search).toLowerCase()) === 0
        );
      });
      console.log(result);
    }
